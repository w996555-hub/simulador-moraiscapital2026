    ...
    <div className="flex h-screen overflow-hidden flex-col bg-background">
      <header className="shrink-0 gradient-primary px-3 md:px-6 shadow-[0_4px_20px_-8px_rgba(180,20,30,0.45)] flex items-center h-16 relative z-20">
        ...
        <div className="ml-auto flex items-center gap-2 md:gap-4 min-w-0">
          <button
            onClick={() => navigateTo('/')}
            className="flex items-center gap-2 px-2.5 md:px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shrink-0"
          >
            <ArrowLeft size={14} /> <span className="hidden sm:inline">Voltar ao Simulador</span>
          </button>
          <span className="hidden md:inline text-white/75 text-sm truncate">Olá, {usuario?.nome?.split(' ')[0] || 'Admin'}</span>
          <button onClick={handleLogout} className="flex items-center justify-center h-9 w-9 bg-white/5 border border-white/15 rounded-full text-white shrink-0">
            <LogOut size={16} />
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 md:px-8 md:py-8 bg-background nice-scroll relative">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between animate-in fade-in duration-500">
            <div>
              <h2 className="text-2xl md:text-[32px] font-bold text-foreground leading-tight">Administração</h2>
